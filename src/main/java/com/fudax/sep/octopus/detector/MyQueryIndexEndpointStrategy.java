...
public class MyQueryIndexEndpointStrategy implements EndpointDetectionStrategy {
    ...
    private static final MediaType MEDIA_TYPE = MediaType.parseMediaType(ActuatorMediaType.V2_JSON);
    ...
    protected Function<ClientResponse, Mono<Endpoints>> convert(Instance instance, String managementUrl) {
        return response -> {
            ...
            if (!response.headers().contentType().map(MEDIA_TYPE::isCompatibleWith).orElse(false)) {
                return response.bodyToMono(Void.class).then(Mono.empty());
            }
    ...
    private boolean filter(String name, String key, Response.EndpointRef endpoint) {
        if (someContextCanPass(name, OctopusConstants.SPECIAL_CONTEXT)) {
            return !"self".equals(key) && !endpoint.isTemplated();
        }
        return !"self".equals(key) && !endpoint.isTemplated() && allowEndpoints.contains(key);
    }
